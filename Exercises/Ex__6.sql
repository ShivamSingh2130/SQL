...
-- STRING OPERATORS: LIKE, REGEXP (REGULAR EXPRESSION), NOT LIKE
...
SELECT Flight_Number, SUBSTRING(Flight_Number, 1,1) AS Flight_Substring FROM Flights; 
...
SELECT Arrival_Airport, RIGHT(Arrival_Airport, 3) AS Last_Character FROM Flights;
...
SELECT AVG(Seats_Available) AS Average_Seats_Available FROM Flights; -- WE CAN RUN THIS QUIERY WITHOUT USING (Average) IT WILL GIVE YOU SAME OUTPUT.


-- 6. Find the Maximum Flight Duration
SELECT MAX(Flight_Duration) AS Longest_Flight_Duration FROM Flights;  -- WE CAN RUN THIS QUIERY WITHOUT USING (LONGEST) IT WILL GIVE YOU SAME OUTPUT.

-- 7. Find the Minimum Flight Duration
SELECT MIN(Flight_Duration) AS Shortest_Flight_Duration FROM Flights; -- WE CAN RUN THIS QUIERY WITHOUT USING (SHORTEST) IT WILL GIVE YOU SAME OUTPUT.

-- 8. Calculate the Total Number of Available Seats Across All Flights
SELECT SUM(Seats_Available) AS Total_Seats_Available FROM Flights; -- WE CAN RUN THIS QUIERY WITHOUT USING (TOTAL) IT WILL GIVE YOU SAME OUTPUT.
...
SELECT AVG(Flight_Duration) AS Average_Flight_Duration FROM Flights; -- WE CAN RUN THIS QUIERY WITHOUT USING (Average) IT WILL GIVE YOU SAME OUTPUT.
...
SELECT SUM(Flight_Duration) AS Total_Flight_Duration FROM Flights;  -- WE CAN RUN THIS QUIERY WITHOUT USING (TOTAL) IT WILL GIVE YOU SAME OUTPUT.
...
-- OR WE CAN USE THIS QIERY FOR SAME OUTPUT.
SELECT COUNT(*) FROM Flights WHERE Seats_Available > 100;
...
-- WE CAN WRITE THISCODE LIKE ..................
SELECT AVG(Seats_Available)  FROM Flights WHERE Status = 'Cancelled';
...
SELECT DATE(Departure_Time) AS Flight_Date, COUNT(*)
FROM Flights 
GROUP BY Flight_Date;
...
SELECT SUM(Flight_Duration)  FROM Flights WHERE Status = 'On Time';
...
SELECT COUNT(*)  FROM Flights WHERE Flight_Duration > 180;
